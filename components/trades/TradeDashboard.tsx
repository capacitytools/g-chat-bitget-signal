...
export function TradeDashboard() {
  ...
  const getLivePnL = (trade: PaperTrade) => {
    ...
    if (trade.direction === "LONG") {
      return ((current - trade.entry) / trade.entry) * (trade.positionSize || 0) * (trade.leverage || 1);
    } else {
      return ((trade.entry - current) / trade.entry) * (trade.positionSize || 0) * (trade.leverage || 1);
    }
  };
        ...
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className={`text-xs mt-1 font-medium ${totalPnl >= 0 ? "text-green-200" : "text-red-200"}`}>
              Realized P/L: {totalPnl >= 0 ? "+" : ""}${(totalPnl || 0).toFixed(2)}
            </p>
            {totalUnrealizedPnL !== 0 && (
              <p className={`text-xs font-medium ${totalUnrealizedPnL >= 0 ? "text-green-200" : "text-red-200"}`}>                Unrealized: {totalUnrealizedPnL >= 0 ? "+" : ""}${(totalUnrealizedPnL || 0).toFixed(2)}
              </p>
            )}
          </div>
        ...
        <div className="space-y-3">
          {openTrades.length === 0 ? (
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-8 text-center">
              <Clock className="w-10 h-10 text-gray-400 mx-auto mb-3" />              <p className="text-sm text-gray-500 dark:text-gray-400">No open paper trades.</p>
            </div>
          ) : (
